...
params:
    ...
    main: "mtcars"
    sub: "Codebook"
    description: "The `mtcars` dadaset contains data extracted from the 1974 *Motor Trend* US magazine, and comprises fuel consumption and 10 aspects of automobile design and performance for 32 automobiles (1973–74 models). It is one of the sample datasets included in base R."
    print_table: FALSE
---
...
codebook_source <- function(data = data, weights = weights, print_table = TRUE) {
  ...
  sortie <- lapply(names(data), function(x) {
    message(paste0(which(names(data) == x), "/", ncol(data), " : ", x))
    cat(paste0("  \n## ", x))
...
```{r codebook, warning = FALSE, message = FALSE, include = TRUE, echo = FALSE, results = "asis", message = FALSE}

# Générer le dictionnaire des variables
codebook_source(data = data, weights = weights, print_table = print_table)

message("Knitting LaTeX output to PDF(may take minutes)...")
```
